const CACHE='nishiharima-guide-v12';
...
self.addEventListener('fetch',e=>{
 ...
 if(e.request.mode==='navigate'){
  e.respondWith(fetch(e.request).then(async resp=>{
   ...
   html=html.replace('</body>','<script src="./admin-enhance.js?v=12"></script><script src="./spot-image-upload.js?v=12"></script><script src="./auth-restore.js?v=12"></script></body>');
   return new Response(html,{status:resp.status,statusText:resp.statusText,headers:resp.headers});
  }).catch(()=>caches.match('./index.html')));
  return;
 }
 const path=new URL(e.request.url).pathname;
 if(path.endsWith('/admin-enhance.js')||path.endsWith('/spot-image-upload.js')||path.endsWith('/auth-restore.js')){e.respondWith(fetch(e.request,{cache:'no-store'}));return;}
